...
## Task 2: WebSocket Protocol Upgrade & Proxying
*   **Goal:** Enable bidirectional, persistent communication channels between clients and downstream services by natively proxying WebSockets.
*   **Definition:**
    1.  Detect the `Upgrade: websocket` HTTP header in incoming requests within the `RequestProxy`.
    2.  If present, bypass the standard `HttpClient.send()` execution path.
    3.  Use Java's native `HttpClient.newWebSocketBuilder()` to establish a connection to the target backend URI (translating `http/https` to `ws/wss`).
    4.  Implement bidirectional pumping: continuously read messages from the client's `InputStream` and send them to the backend WebSocket, while concurrently listening for backend WebSocket messages and writing them to the client's `OutputStream`.
*   **Reasoning:** WebSockets use a distinct framing protocol after the initial HTTP handshake. The standard HTTP proxying mechanism will fail or drop the connection. A dedicated upgrade and bridging tunnel is required for modern real-time APIs.

## Task 3: Active Health Checking Daemon
...
## Task 4: Load Balancer Health Integration
...
## Task 5: Actuator Health Visibility
...
## Task 6: Integrate Resilience4j Circuit Breakers
...
## Task 7: Header Transformation & Manipulation
...
## Task 8: URL Rewriting & Prefix Stripping
...
## Task 9: Core RED Metrics Instrumentation (Prometheus)
...
*   **Definition:**
    ...
    2.  Write an `*IT.java` test that configures WireMock to return `500 Server Error` consistently to verify the Circuit Breaker (Task 6) opens and traffic is rerouted.
    3.  Write tests validating Header Transformation (Task 7) and URL Rewriting (Task 8) rules.
